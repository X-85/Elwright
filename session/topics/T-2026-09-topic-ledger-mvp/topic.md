...
- `topic-ledger/` 可单独交给 ZCode 安装，不依赖 Elwright 运行时。
- ZCode 负责安装源 Skill，不额外提供安装脚本。
- 零依赖本地查看页。
- 保留现有 Q 编号和旧台账兼容性。

## 完成标准
- 明天在公司 Windows 把源 Skill 交给 ZCode 后，可以在 ZCode 中维护一个 Topic。
- 选择 Topic 文件夹后，查看页能显示目标、当前状态、决定、未决项、产出物和交接摘要。

## 当前摘要
已确定 Topic 是人的工作单元，Chat 是交流容器，Task 是执行记录。MVP 采用普通 Markdown 文件协议，不绑定 Elwright 或某个 Agent。独立源 Skill 和 Chrome 查看页已落地，安装动作交给 ZCode，等待公司机真实使用。

## 已验证
- [已验证] 现有 `session-ledger` 使用 `index.md`、`events.md`、`decisions.md` 和 `archive/`。
- [已验证] Elwright 现有“课题”页面是资源研究模型，不直接复用为工作 Topic。
- [已验证] `topic-ledger/` 目录可单独携带，包内不依赖 Elwright 运行时、Node.js 或安装脚本。
- [未验证] 公司 Windows 的 ZCode Skill 目录实际路径和安装后自动发现行为。
- [未验证] 公司 Chrome 双击查看页和文件夹选择器的真实行为。

## 未决问题
- [ ] 公司 ZCode 能否直接安装或加载源 `SKILL.md`。
- [ ] 明天真机使用后，是否需要把 Skill 名从 `topic-ledger` 改成 `session-ledger`。
...
- `topic-ledger/README.md`：源 Skill 交给 ZCode 和 Chrome 查看说明。
