...
- `git diff --check`：通过。
- 独立分发检查：`topic-ledger/` 内无 Elwright 运行时、注册表或 Node 依赖；复制该目录即可执行安装脚本和打开查看页。
- PowerShell 静态检查：安装脚本在兼容旧 `session-ledger` 名称时显式按 UTF-8 读取 Skill，避免 Windows PowerShell 5.1 中文乱码。
