# 变更日志（Changelog）
## 2026-08-31 · i18n 基建与界面语言（ADR-002）

- 新增 `lib/i18n.ts` 轻量 t() + zh-CN/en 双语字典（零依赖，键集完整性测试守护）。
- preferences 新增 `language` 字段（默认 zh-CN），切换即时生效并持久化。
- 「界面语言」选择器启用（中文 / English）；设置中心壳层字符串全部走 t()。
- 增量迁移遗留：枚举选项标签（启动视图/密度/字体名等）与 LlmSettings 内部文案。
