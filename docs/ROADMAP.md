...
v0.1.8（2026-08-31，tag `v0.1.8`，GitHub Release 附 dmg + msi。基于 v0.1.7 新增：代码浏览器阶段①（本地项目只读查看 + 轻量符号跳转，见已完成里程碑 2026-08-31 条目）。上一版：v0.1.6 发布后 1 小时真机冒烟发现资源收藏 IPC 恒失败（Resource.id 缺 serde(default)），本版修复并补真协议回归测试。原 v0.1.6 内容：基于 v0.1.5 新增：渐进式能力成长 MVP（成熟度档位/核心能力视图/本地使用计数）+ 资源课题工作区与软件快捷方式 + 人与人消息会话一期 + 会话问题台账能力（内置注册表 3→4 条）+ app-chrome 壳演进（窗口红绿灯/布局菜单）+ 工作台第一阶段（Todo + 今日记录）+ 工程质量第二档（分层 e2e））
...
- **设置中心后续阶段**：~~第一阶段（常规/外观/模型分组 + 三态主题）~~（已随 PR #1 进 main）；余下终端主题、字体和字号同步；常规中的启动视图、更新策略与语言；模型档案等配置，均以本地优先、少而明确为准入标准。
