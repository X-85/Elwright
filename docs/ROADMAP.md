...
- **工作工具栏第一阶段**（`feature-2026-08-workbench-phase1`）：Todo 清单 + 今日记录（轻量记事本），纯本地用户层存储（`~/.elwright/todos.json` + `notes/`）。范围调整（2026-08-25 用户拍板）：今日记录由「后置」提前进第一阶段，收藏/最近使用、书签、高频转换工具顺延为后续阶段。
...
- **渐进式能力成长系统 MVP**（`codex/feature-2026-08-progressive-capabilities`）：能力成熟度档位、默认核心能力视图、全部能力浏览开关和本地使用计数；社区共建与远端能力包后置。
- **会话问题台账基准技能**（`feature-2026-08-session-ledger`）：内置 `session-ledger` 与离线 SOP，同时安装 Codex `$session-ledger`，明确 Elwright 文本生成和 Codex 文件维护的边界。
- **浏览器端到端冒烟测试**（`feature-2026-08-playwright-smoke`）：Playwright 覆盖关键按钮和预览工作流，使用隔离浏览器数据；Tauri 原生功能另设桌面壳测试。
...
- **资源管理与课题工作区**（`codex/feature-2026-08-progressive-capabilities`，含 `enhancement-2026-08-software-shortcuts`）：本地三层收藏夹、网页/文档/能力/笔记资源、图标化软件快捷方式、资源聚合课题与 LLM/离线报告草稿。
