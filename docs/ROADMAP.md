...
- **AI 对话 阶段②（会话管理）**（feature-2026-08-chat-phase2）：会话侧栏（新建/切换/重命名/删除）+ `~/.elwright/chats/` 一文件一会话 + 自动保存 + 错误消息不落盘；零新依赖（手写 ISO8601 时间戳 + AtomicU64 id）。代码完成，自动化验证过（cargo 38 测试/构建/IAB 预览布局与降级），**待真机 GUI 验证**（多会话切换/重命名/删除/重启后恢复）。
