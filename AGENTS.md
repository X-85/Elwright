...
看 `docs/ROADMAP.md`（活文档，唯一排期视图）。一句话现状：v0.1.5 已发；工程质量第二档（分层 e2e 冒烟）已并入 main；工作台第一阶段（Todo + 今日记录）验证通过待合并；进行中的功能分支（渐进式能力成长 / 设置中心一期 / 消息一期等）等合并积压清空后依次 rebase。资源工具脚本约定：**纯 Python 标准库、零第三方依赖、中文报错**——离网可跑是底线。
