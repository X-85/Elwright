...
看 `docs/ROADMAP.md`（活文档，唯一排期视图）。一句话现状：v0.1.4 已发（集成终端 v1 + AI 对话阶段①② + 一键安装脚本入包，dmg+msi 已上 Release）；进行中 = 应用壳布局第一阶段（`enhancement-2026-08-app-shell-layout`，顶部全局栏 + 面板显隐）。资源工具脚本约定：**纯 Python 标准库、零第三方依赖、中文报错**——离网可跑是底线。
