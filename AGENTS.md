...
看 `docs/ROADMAP.md`（活文档，唯一排期视图）。一句话现状：v0.1.1 已发；集成终端 v1 真机验证通过、待随 v0.1.2 发版；进行中 = V2 AI 对话阶段①（对话基础，代码完成待真机验证，`docs/work/active/feature-2026-08-chat-phase1/`）与阶段②（会话管理，代码完成待真机验证，`docs/work/active/feature-2026-08-chat-phase2/`）。资源工具脚本约定：**纯 Python 标准库、零第三方依赖、中文报错**——离网可跑是底线。
