...
看 `docs/ROADMAP.md`（活文档，唯一排期视图）。一句话现状：v0.1.5 已发；PR #1 大批次（渐进式能力成长 MVP / 资源课题工作区 / 消息一期 / 台账能力 / app-chrome 壳）已于 2026-08-30 合并 main 且 CI 全绿，待发版；进行中暂无，下一站代码浏览器阶段①。资源工具脚本约定：**纯 Python 标准库、零第三方依赖、中文报错**——离网可跑是底线。
