...
看 `docs/ROADMAP.md`（活文档，唯一排期视图）。一句话现状：v0.1.1 已发；集成终端 v1 + AI 对话阶段①② 均已真机验证通过、合并在 `feat/chat` 待发版；进行中暂空。下一步 = 合并 `feat/chat` 后打 v0.1.3 tag（用户动作）。资源工具脚本约定：**纯 Python 标准库、零第三方依赖、中文报错**——离网可跑是底线。
