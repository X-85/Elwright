...
✅ **v0.1.11 已发布**（进度详情看 [docs/ROADMAP.md](docs/ROADMAP.md)，此处只留概览）：桌面壳已带能力工具箱、AI 对话（多轮/会话/能力协作/流式与取消）、代码浏览器（只读浏览/符号跳转/收藏书签/受控补丁编辑）、工作台（Todo/今日记录）、资源与课题工作区、人与人消息会话一期、设置中心（常规/外观/终端/模型档案）、集成终端、CLI `ew`。主干红线与下一步排期以 ROADMAP 为准；内置注册表保留 3 个真实示例（文本统计、能力类型说明、周报生成），个人能力通过导入加入 `~/.elwright/`。
